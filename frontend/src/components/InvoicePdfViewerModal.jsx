...
import { Printer, Download, X, CheckCircle, FileText, Building } from 'lucide-react';
...
export default function InvoicePdfViewerModal({ isOpen, onClose, invoice }) {
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-content" style={{ maxWidth: '780px', maxHeight: '90vh', overflowY: 'auto', padding: '1.75rem' }}>
        {/* Header Controls */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <button className="btn btn-primary" onClick={handlePrint} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.8rem', padding: '0.35rem 0.75rem' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #0f172a', paddingBottom: '1rem', marginBottom: '1.25rem' }}>
            <div>
              <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#4f46e5' }}>OPS SaaS Systems India Pvt Ltd</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem', background: '#f8fafc', padding: '1rem', borderRadius: '6px' }}>
            <div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '1.25rem', fontSize: '0.8rem' }}>
            ...
            <tbody>
              <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                ...
                <td style={{ padding: '0.65rem', textAlign: 'right', fontWeight: 700 }}>₹{invoice.baseAmount?.toFixed(2)}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1.5rem' }}>
            <div style={{ width: '320px', background: '#f8fafc', padding: '1rem', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
              ...
              {isIntraState ? (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.35rem', fontSize: '0.78rem', color: '#475569' }}>
                    <span>CGST (9.0%):</span>
              ...
              <div style={{ borderTop: '2px solid #cbd5e1', paddingTop: '0.5rem', marginTop: '0.5rem', display: 'flex', justifyContent: 'space-between', fontSize: '1rem', fontWeight: 800, color: '#0f172a' }}>
                <span>Total Invoice Value:</span>
                <span style={{ color: '#4f46e5' }}>₹{invoice.totalAmount?.toFixed(2)}</span>
              </div>
            </div>
          </div>

          {/* Bank & Remittance Instructions */}
          <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '1rem', display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '1rem', fontSize: '0.75rem', color: '#64748b' }}>
            <div>
              <div style={{ fontWeight: 700, color: '#334155', marginBottom: '0.25rem' }}>Bank Transfer / RTGS / NEFT:</div>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontWeight: 700, color: '#334155', marginBottom: '0.25rem' }}>Terms & Conditions:</div>
